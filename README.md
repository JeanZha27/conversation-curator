...
当前报告 `schemaVersion` 为 `1.4`。相较 `1.3`，隐私元数据新增
`crossRunLinkable: true`，明确说明确定性源文件哈希和对话引用可在不同报告之间关联。
此前相较 `1.1` 新增的汇总字段包括
...
输入必须是 `.json` 顶层对象数组，首个非空元素还必须包含 ChatGPT 会话的稳定 ID 与 `mapping`。CLI 只打开源路径一次，在复制过程中强制执行 256 MiB 上限，并将捕获到的字节写入权限为 `0600` 的临时快照；结构预检、哈希和完整解析都使用该快照。不符合结构时返回 `INPUT_NOT_CHATGPT_EXPORT`。CLI 成功返回 0；空结果、重复 ID 或部分失败返回 2；校验及运行错误返回 1；取消返回 130。

## 隐私模型

- 输入源只读打开一次；后续处理使用有界临时快照，不会因路径被替换而切换到另一份文件。快照在运行结束时删除，清理失败会明确报错。
...
- 报告保存分组文件哈希、单向对话引用、统计和分类字段，但不保存消息正文、原始标题、源对话 ID 或敏感匹配值。文件哈希和对话引用是确定性的，因此持有两份报告的人可以判断它们是否来自同一源文件或包含同一会话；`privacy.crossRunLinkable` 固定为 `true`，报告仍应按私密数据处理。
...
- 对话数量上限：50,000。
- 单个对话对象上限：8 MiB。
- 每个对话的 `mapping` 节点上限：50,000；超过后在展开或排序前安全失败。
- 单条分类文本超过 32 Ki 字符时仅保留首尾片段并强制人工复核；确定性敏感扫描仍覆盖完整文本。
- 最近一次本机数量基准：50,000 条、15,316,673 字节输入、50,950,678 字节输出；峰值 RSS 126.2 MiB。
- 最近一次本机近上限基准：4,000 条、261,212,673 字节输入（约 249.1 MiB）、4,148,678 字节输出；峰值 RSS 121.2 MiB。
- 最近一次本机大对象基准：30 条、225,008,913 字节输入（单条正文约 7.15 MiB）、31,778 字节输出；峰值 RSS 173.5 MiB。
- 最近一次本机节点边界基准：单项 50,001 个微小 `mapping` 节点、1,438,963 字节输入；按预期返回 `MAPPING_NODE_LIMIT_EXCEEDED`，峰值 RSS 128.2 MiB。

四项结果均低于 256 MiB 基准阈值，但只证明上述合成数据形态，不代表所有真实导出都具有相同内存曲线。
...
[GOVERNANCE.md](GOVERNANCE.md) 说明当前维护者、独立复核边界、版本和签名发布规则。
